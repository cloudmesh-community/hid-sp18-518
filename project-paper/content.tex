...
following functions	& libcloud.get\_driver()\\
 ...
 Node listing & Used to enumerate the nodes already created within the cloud provider & list\_nodes()\\
 Size listing & Used to enumerate the size of the VM and supporting components & list\_sizes()\\
 Image listing & Used to enumerate the avaiable, deployable images the cloud provider makes available & list\_images()\\
 Get Image ID & Used to pull the unique identifier for the human-readable name for the chosen image & get\_image(\\
 Get Node ID & Used to pull the unique identifier for one or many VMs & ex\_get\_node()\\
 Create new node & Used in conjuection with image listing to deploy a new VM & create\_node()\\
 Start a node & Used to tell a previously built VM to boot & ex\_start\_node()\\
 Stop a node & Used to direct a VM to begin shutdown gracefully & ex\_stop\_node()\\
 Destroy a node & Used to delete a VM and will return success & destroy\_node()\\
 Get Volume ID & Used to enumerate the unique identifier for cloud storage typically used by a VM & ex\_get\_volume()\\
 Create new volume & Used to add addtional storage to a VM & create\_volume()\\
 Create new snapshot & Used to take a recovery image of a VM for restoration purposes & create\_volume\_snapshot()\\
 Destroy a volume or snapshot & Used to delete storage or snapshot to recover storage space & destroy\_volume()\\
  \hline
 \end{tabular}%
 }
 \end{table}
