...
package using the Python package management system, pip. To use libcloud as a library,
leverage Yython and import libcloud like any other library. A useful feature to use in 
...
to use that account. Next, local variables were defined to store the credentials to be used.
Once that was set up, we defined the EC2 driver with our region preference, which was 
AWS us-east-1, the most full featured region. This is because there are additional tasks 
that must be taken with SSH keys. We used our browser to review EC2 and finalize the setup.
